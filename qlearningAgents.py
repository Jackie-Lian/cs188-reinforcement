...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        if self.q_vals.get((state, action)) is None:
            return 0.0
        else:
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(actions) == 0:
            ...
            if best_q < 0:
                seen_actions = []
                unseen_actions = []
                for s, a in self.q_vals:
                    if s == state:
                        seen_actions.append(a)
                for action in actions:
                    if action not in seen_actions:
                        unseen_actions.append(action)
                if len(unseen_actions) != 0:
                    best_action = random.choice(unseen_actions)
            return best_action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features:
            q_val = q_val + features[feature] * self.weights[feature]
        return q_val
